...
object StressChaos {
  ...
  final case class MemoryStressor(
    ...
    /**
     * Specifies the memory size to be occupied or a percentage of the total memory size. The final
     * sum of the occupied memory size. Like 256MB / 25%
     */
    def withOccupiedSize(size: String) =
      copy(size = size.some)

    /**
     * Specifies oom score value, which defines probability to be killed by OOM. -1000 (very
     * unlikely to be killed by the OOM killer) up to 1000 (very likely to be killed by the OOM
     * killer)
     */
